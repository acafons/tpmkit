...
[[nodiscard]] outcome<tpm_context_config> validate(tpm_context_config config)
{
    if (const auto* const string_tcti = std::get_if<tcti_string_config>(&config.tcti)) {
        if (string_tcti->config.empty()) {
            return tl::unexpected(error{
                error_category::input_error,
                "TCTI configuration string must not be empty"});
        }
    }

    if (const auto* const owned_tcti = std::get_if<tcti_owned_handle>(&config.tcti)) {
        if (owned_tcti->handle == nullptr) {
            return tl::unexpected(error{
                error_category::input_error,
                "Owned TCTI handle must not be null"});
        }
    }
...
fake_tpm_context::fake_tpm_context(tpm_context_config config) noexcept
