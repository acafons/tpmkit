...
mock_pcr_provider::mock_pcr_provider()
    : allocate_result_{make_default_failure<pcr_allocate_result>()},
      event_result_{make_default_failure<pcr_event_result>()},
      extend_result_{make_default_void_failure()},
      read_result_{make_default_failure<pcr_read_result>()},
...
outcome<pcr_allocate_result> mock_pcr_provider::allocate(gsl::span<const pcr_bank>)
{
...
outcome<pcr_event_result> mock_pcr_provider::event(pcr_index, gsl::span<const std::uint8_t>)
{
    ++event_call_count_;
...
outcome<void> mock_pcr_provider::extend(pcr_index, gsl::span<const pcr_digest_value>)
{
    ++extend_call_count_;
...
outcome<pcr_read_result> mock_pcr_provider::read(const pcr_selection&)
{
    ++read_call_count_;
    return read_result_;
}
...
outcome<void> mock_pcr_provider::reset(pcr_index)
{
    ++reset_call_count_;
...
void mock_pcr_provider::set_allocate_result(outcome<pcr_allocate_result> result)
{
    allocate_result_ = std::move(result);
}

outcome<void> mock_pcr_provider::set_auth_policy(pcr_index, hash_algorithm,
...
outcome<void> mock_pcr_provider::set_auth_value(pcr_index, secret_buffer)
{
...
void mock_pcr_provider::set_event_result(outcome<pcr_event_result> result)
{
...
void mock_pcr_provider::set_read_result(outcome<pcr_read_result> result)
{
