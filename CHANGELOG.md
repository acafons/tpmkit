...
- Add CMake policy guards for domain isolation and backend-neutral public API boundaries.

### Changed

- **Breaking:** `tpm_context_config::tcti` is now backend-neutral and stores only a string TCTI configuration. Migration: use `<tpmkit/tpm2_esys/owned_tcti_context.h>` and `tpmkit::tpm2_esys::create_context(...)` when transferring an owned `TSS2_TCTI_CONTEXT`.
...
- Pin OpenSSL 3.5.5 and spdlog 1.15.3 consistently in CMake/vcpkg metadata.
