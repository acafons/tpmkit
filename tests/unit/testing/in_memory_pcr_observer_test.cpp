...
static_assert(noexcept(std::declval<tpmkit::testing::in_memory_pcr_observer&>().on_extend(
                  std::declval<tpmkit::pcr::index>(),
                  std::declval<gsl::span<const tpmkit::pcr::digest_value>>())),
...
static_assert(noexcept(std::declval<tpmkit::testing::in_memory_pcr_observer&>().on_event(
                  std::declval<tpmkit::pcr::index>(), std::declval<gsl::span<const std::uint8_t>>(),
                  std::declval<const tpmkit::pcr::event_result&>())),
...
TEST(in_memory_pcr_observer, on_extend_records_correct_index_and_digests)
{
    ...
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x11U)}};

    observer.on_extend(tpmkit::pcr::index::debug,
    ...
    const std::vector<tpmkit::pcr::digest_value> expected_digests{digests.begin(), digests.end()};
    EXPECT_EQ(record.index, tpmkit::pcr::index::debug);
...
TEST(in_memory_pcr_observer, on_event_records_correct_index_event_data_and_result)
{
    ...
    const tpmkit::pcr::event_result result{{make_sha256_digest(0x22U)}};

    observer.on_event(tpmkit::pcr::index::application, gsl::span<const std::uint8_t>{event_data},
                      result);

    ASSERT_EQ(observer.count(), 1U);
    const tpmkit::testing::pcr_measurement_record& record = observer.entries().front();
    const std::vector<std::uint8_t> expected_event_data{event_data.begin(), event_data.end()};
    EXPECT_EQ(record.index, tpmkit::pcr::index::application);
...
TEST(in_memory_pcr_observer, entries_returns_all_recorded_measurements_in_order)
{
    ...
    tpmkit::testing::in_memory_pcr_observer observer;
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x33U)}};
    const tpmkit::pcr::event_result result{{make_sha256_digest(0x44U)}};

    observer.on_extend(tpmkit::pcr::index::debug,
                       gsl::span<const tpmkit::pcr::digest_value>{digests});
    observer.on_event(tpmkit::pcr::index::application, gsl::span<const std::uint8_t>{}, result);
    ...
    EXPECT_EQ(observer.entries()[0].index, tpmkit::pcr::index::debug);
    EXPECT_EQ(observer.entries()[1].index, tpmkit::pcr::index::application);
}
...
TEST(in_memory_pcr_observer, entries_by_index_returns_matching_records)
{
    ...
    tpmkit::testing::in_memory_pcr_observer observer;
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x55U)}};

    observer.on_extend(tpmkit::pcr::index::debug,
                       gsl::span<const tpmkit::pcr::digest_value>{digests});
    observer.on_extend(tpmkit::pcr::index::application,
                       gsl::span<const tpmkit::pcr::digest_value>{digests});
    observer.on_extend(tpmkit::pcr::index::debug,
                       gsl::span<const tpmkit::pcr::digest_value>{digests});

    const std::vector<tpmkit::testing::pcr_measurement_record> records =
        observer.entries_by_index(tpmkit::pcr::index::debug);

    ASSERT_EQ(records.size(), 2U);
    EXPECT_EQ(records[0].index, tpmkit::pcr::index::debug);
    EXPECT_EQ(records[1].index, tpmkit::pcr::index::debug);
}
...
TEST(in_memory_pcr_observer, clear_resets_recording)
{
    ...
    tpmkit::testing::in_memory_pcr_observer observer;
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x66U)}};
