...
[[nodiscard]] tpmkit::pcr::digest_value make_sha256_digest(const std::uint8_t fill)
{
    return tpmkit::pcr::digest_value{tpmkit::hash_algorithm::sha256,
                                     std::vector<std::uint8_t>(32U, fill)};
}

[[nodiscard]] tpmkit::pcr::read_result make_read_result()
{
    return tpmkit::pcr::read_result{
        tpmkit::pcr::selection{tpmkit::hash_algorithm::sha256, {tpmkit::pcr::index::debug}},
        7U,
        {tpmkit::pcr::value{tpmkit::pcr::index::debug, make_sha256_digest(0xA5U)}}};
}
...
TEST(mock_pcr_provider, default_response_returns_backend_error_for_each_operation)
{
    ...
    const std::array<tpmkit::pcr::bank, 1> banks{
        {tpmkit::pcr::bank{tpmkit::hash_algorithm::sha256}}};
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x01U)}};
    ...
    const auto extend = provider.extend(tpmkit::pcr::index::debug,
                                        gsl::span<const tpmkit::pcr::digest_value>{digests});
    const auto event =
        provider.event(tpmkit::pcr::index::debug, gsl::span<const std::uint8_t>{event_data});
    const auto reset = provider.reset(tpmkit::pcr::index::debug);
    const auto allocate = provider.allocate(gsl::span<const tpmkit::pcr::bank>{banks});
    const auto set_auth_value =
        provider.set_auth_value(tpmkit::pcr::index::debug, tpmkit::secret_buffer{});
    const auto set_auth_policy =
        provider.set_auth_policy(tpmkit::pcr::index::debug, tpmkit::hash_algorithm::sha256,
...
TEST(mock_pcr_provider, programmed_success_response_is_returned_for_read)
{
    ...
    tpmkit::testing::mock_pcr_provider provider;
    const tpmkit::pcr::read_result expected = make_read_result();
...
TEST(mock_pcr_provider, programmed_success_response_is_returned_for_extend)
{
    ...
    tpmkit::testing::mock_pcr_provider provider;
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x02U)}};
    provider.set_extend_result({});

    const auto actual = provider.extend(tpmkit::pcr::index::debug,
                                        gsl::span<const tpmkit::pcr::digest_value>{digests});
...
TEST(mock_pcr_provider, programmed_error_response_is_returned_correctly)
{
    ...
    const auto actual = provider.event(tpmkit::pcr::index::debug, gsl::span<const std::uint8_t>{});
...
TEST(mock_pcr_provider, tracks_call_count_for_each_operation)
{
    ...
    const std::array<tpmkit::pcr::bank, 1> banks{
        {tpmkit::pcr::bank{tpmkit::hash_algorithm::sha256}}};
    const std::array<tpmkit::pcr::digest_value, 1> digests{{make_sha256_digest(0x03U)}};

    static_cast<void>(provider.allocate(gsl::span<const tpmkit::pcr::bank>{banks}));
    static_cast<void>(provider.event(tpmkit::pcr::index::debug, gsl::span<const std::uint8_t>{}));
    static_cast<void>(provider.extend(tpmkit::pcr::index::debug,
                                      gsl::span<const tpmkit::pcr::digest_value>{digests}));
    static_cast<void>(provider.read(tpmkit::pcr::selection{tpmkit::hash_algorithm::sha256}));
    static_cast<void>(provider.reset(tpmkit::pcr::index::debug));
    static_cast<void>(provider.set_auth_policy(tpmkit::pcr::index::debug,
                                               tpmkit::hash_algorithm::sha256,
                                               gsl::span<const std::uint8_t>{}));
    static_cast<void>(provider.set_auth_value(tpmkit::pcr::index::debug, tpmkit::secret_buffer{}));
