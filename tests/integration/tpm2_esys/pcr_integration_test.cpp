...
[[nodiscard]] provider_bundle make_provider(tpmkit::pcr::observer* const observer = nullptr,
...
[[nodiscard]] tpmkit::pcr::digest_value digest_value(const tpmkit::hash_algorithm algorithm,
                                                     const std::uint8_t fill)
{
    return tpmkit::pcr::digest_value{
        algorithm, std::vector<std::uint8_t>(tpmkit::digest_size(algorithm), fill)};
}

[[nodiscard]] tpmkit::outcome<std::vector<std::uint8_t>>
read_debug_digest(tpmkit::pcr::provider& provider, const tpmkit::hash_algorithm algorithm)
{
    auto read = provider.read(tpmkit::pcr::selection{algorithm, {tpmkit::pcr::index::debug}});
    if (!read.has_value()) {
...
[[nodiscard]] std::vector<tpmkit::pcr::index> auth_candidate_indices()
{
    return {tpmkit::pcr::index::debug,      tpmkit::pcr::index::application,
            tpmkit::pcr::index::drtm_17,    tpmkit::pcr::index::drtm_18,
            tpmkit::pcr::index::drtm_19,    tpmkit::pcr::index::drtm_20,
            tpmkit::pcr::index::drtm_21,    tpmkit::pcr::index::drtm_22,
            tpmkit::pcr::index::firmware_0, tpmkit::pcr::index::firmware_1,
            tpmkit::pcr::index::firmware_2, tpmkit::pcr::index::firmware_3,
            tpmkit::pcr::index::firmware_4, tpmkit::pcr::index::firmware_5,
            tpmkit::pcr::index::firmware_6, tpmkit::pcr::index::firmware_7};
}

[[nodiscard]] const tpmkit::pcr::digest_value*
find_digest(const std::vector<tpmkit::pcr::digest_value>& values,
...
[[nodiscard]] tpmkit::outcome<tpmkit::pcr::index>
set_auth_policy_on_supported_pcr(tpmkit::pcr::provider& provider,
                                 const gsl::span<const std::uint8_t> policy_digest)
{
    ...
    for (const tpmkit::pcr::index index : auth_candidate_indices()) {
        auto result =
...
[[nodiscard]] tpmkit::outcome<tpmkit::pcr::index>
set_auth_value_on_supported_pcr(tpmkit::pcr::provider& provider,
...
TEST(pcr_provider_swtpm, extends_debug_pcr_and_reads_chained_sha256_value)
{
    ...
    auto bundle = make_provider();
    const auto reset = bundle.provider->reset(tpmkit::pcr::index::debug);
    ...
    const std::array<tpmkit::pcr::digest_value, 1U> first{first_digest};

    const auto first_extend = bundle.provider->extend(
        tpmkit::pcr::index::debug, gsl::span<const tpmkit::pcr::digest_value>(first));
    ...
    const auto second_extend = bundle.provider->extend(
        tpmkit::pcr::index::debug, gsl::span<const tpmkit::pcr::digest_value>(second));
...
TEST(pcr_provider_swtpm, resets_debug_pcr_to_zero)
{
    ...
    const auto extend = bundle.provider->extend(
        tpmkit::pcr::index::debug, gsl::span<const tpmkit::pcr::digest_value>(digests));
...
TEST(pcr_provider_swtpm, observer_records_extend_and_event_measurements)
{
    ...
    EXPECT_EQ(entries[0].index, tpmkit::pcr::index::debug);
    EXPECT_EQ(entries[0].digests, std::vector<tpmkit::pcr::digest_value>{digest});
...
TEST(pcr_provider_swtpm, allocates_sha384_bank_when_available)
{
    ...
    std::transform(
        ...
        [](const tpmkit::hash_algorithm algorithm) { return tpmkit::pcr::bank{algorithm}; });
