...
- **Logged at adapter boundaries:** errors from OpenSSL and TSS, before translation to domain errors. Include the original error code, sanitized decoded backend diagnostic text when available, and library context, but never secret-derived bytes (no "expected MAC X, got Y").
