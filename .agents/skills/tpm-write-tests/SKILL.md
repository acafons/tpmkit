...
9. **Keep the policy guard green.** Every test change must pass the `test_policy_guard` CTest entry. The guard fails missing top-of-test behavior comments, non-parameterized contract tests, `GTEST_SKIP()` in unit/contract/integration tests, deferred skip placeholders, compile-only header smoke tests placed under `tests/integration/`, and TPM2 ESYS adapter-unit regressions such as direct TSS calls, byte-scripted fake TCTI command queues, or concurrency stress cases under `tests/unit/tpm2_esys/`.
...
- **Adapter-internal unit tests** — pure translation, validation, schema, and ABI-shim logic for a specific adapter. Place these under `tests/unit/<adapter>/` (for example `tests/unit/tpm2_esys/`); grouped adapter families mirror the source layout, as logging does under `tests/unit/logging/<backend>/`. They may compile against the backend SDK when needed for constants or function signatures, but they do not open real backend resources or call vendor SDK entry points. Introduce an adapter-internal function table or stub seam and assert the structured arguments captured at that seam. Anything that needs swtpm, hardware, a byte-accurate transport transcript, or a real library call belongs in the integration tier or contract fake-backend harness, not in adapter-unit tests.
...
- All external dependencies are mocked. No network, no filesystem, no real OpenSSL/TSS calls — those belong in the integration tier. Adapter-internal and adapter-shaped testing-helper unit tests may include third-party headers or link SDK libraries only to compile constants, types, or ABI-compatible fake handles; they must not exercise a live backend or use real SDK entry points as part of the behavior under test.
- Adapter-unit tests verify the adapter's structured boundary contract. For TPM2 ESYS this means asserting `ESYS_TR` handles, sessions, `TPML_*` selections/digests, authorization buffers, returned domain values, and cleanup behavior captured by the fake API. Do not assert on raw transmitted command bytes, byte substrings, or fake transport queue lengths as a proxy for adapter behavior.
- Log assertions must prove the schema, not merely that a record exists. Assert the event name, level, outcome, operation, required field keys, exact field values where stable, and the absence of secret/caller-sensitive material. If the test uses a fake backend error decoder, assert the fake decoded text so the unit test cannot accidentally call the real decoder.
- Concurrency and load behavior for TPM adapters is not a unit concern. Tests that create threads around an ESYS context, stress session/handle lifetimes, or depend on scheduler interleavings belong in `tests/stress/` or a swtpm-backed integration test with a clear fixture.
...
* **`test_policy_guard` fails.** Fix the test shape rather than relaxing the guard. Missing top comments need a one-sentence behavior comment inside the test body; contract tests must be `TEST_P` plus `INSTANTIATE_TEST_SUITE_P`; deferred `GTEST_SKIP()` placeholders must become real tests or be removed; compile-only header smoke sources belong in `tests/unit/public_api/` or a dedicated compile-smoke target, not `tests/integration/`. For TPM2 ESYS adapter-unit failures, move real TSS calls to integration tests, replace fake TCTI byte scripts with injected fake API seams, and move threaded stress coverage out of `tests/unit/tpm2_esys/`.
